...
def _geglu_tanh_backward_kernel(dc, a, b, stride, n_cols: tl.constexpr, BLOCK_SIZE: tl.constexpr
    ...
    for sub_block_idx in range(num_sub_blocks):
        ...
        dc_row = tl.load(dc + base_offset + col_offsets, mask=mask, other=0)
        a_row = tl.load(a + base_offset + col_offsets, mask=mask, other=0).to(tl.float32)
        b_row = tl.load(b + base_offset + col_offsets, mask=mask, other=0)
        ...
        tl.store(a + base_offset + col_offsets, da_row, mask=mask)
        tl.store(b + base_offset + col_offsets, db_row, mask=mask)
...
def geglu_forward(a, b):
    ...
    _geglu_tanh_forward_kernel[(n_rows,)](
        ...
        BLOCK_SIZE_SUB = 4096 if is_npu_available() else BLOCK_SIZE,
        num_warps=num_warps,
    )
